...
const actions = {
    getArtists: async (context, payload) => {
        let artists  = await axios.get('http://127.0.0.1:8000/agogy/artists')
        context.commit('setArtists', artists.data.artists)
    },
    getApplicationsArtists: async (context, payload) => {
        let applications = await axios.get('http://127.0.0.1:8000/agogy/applications/artists', {
            headers: {
    ...
    showArtist: async (context, payload) => {
        let artist = await axios.get(`http://127.0.0.1:8000/agogy/artists/${payload}`, {
            headers: {
                "Content-Type": "application/json",
                "Authorization": 'Bearer ' + localStorage.getItem('auth_token')
            }
    ...
    setConfirmArtist: async (context, payload) => {
        await axios.post(`http://127.0.0.1:8000/agogy/artists/${payload}`, {},{
            headers: {
                "Authorization": 'Bearer ' + localStorage.getItem('auth_token')
            }
        }).then(response => {
    ...
    createApplicationArtist: async (context) => {
        await axios.post('http://127.0.0.1:8000/agogy/artist/application', {},{
            headers: {
                "Content-Type": "application/json",
                "Authorization": 'Bearer ' + localStorage.getItem('auth_token')
            }
    ...
    setRejectArtist: async (context, payload) => {
        await axios.delete(`http://127.0.0.1:8000/agogy/artists/${payload}`, {
            headers: {
                "Content-Type": "application/json",
                "Authorization": 'Bearer ' + localStorage.getItem('auth_token')
            }
